...
def browse_directory(directory):
    ...
    # Ensure the target directory is within the allowed base directory
    if not os.path.commonpath([base_dir]) == os.path.commonpath([base_dir, target_dir]):
        abort(403)  # Forbidden
    ...
    try:
        ...
        return jsonify({'path': directory, 'items': items})
    except Exception as e:
